  ...
  return (
    <div className="flex flex-row justify-between">
      <Tooltip text="Previous_Project" disabled={!hasPrevProject}>
        <Link href={hasPrevProject ? `/m/projects/${prevProjectSlug}` : ""}>
          <div
      ...
      <Tooltip text="Next_Project" disabled={!hasNextProject}>
        <Link href={hasNextProject ? `/m/projects/${nextProjectSlug}` : ""}>
          <div
            className={`border text-center border-dotted bborder-dark dark:border-gray text-2xl hover:border-solid bg-light dark:bg-black hover:bg-white dark:hover:bg-blacks py-4 px-4 transition-all ease-in-out duration-1000 ${!hasNextProject ? "opacity-30 cursor-not-allowed" : ""}`}
